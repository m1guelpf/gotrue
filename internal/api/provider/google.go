...
func NewGoogleOIDCProvider(ctx context.Context) (*oidc.Provider, error) {
	return oidc.NewProvider(ctx, IssuerGoogle)
}
...
func (g googleProvider) GetUserData(ctx context.Context, tok *oauth2.Token) (*UserProvidedData, error) {
	if idToken := tok.Extra("id_token"); idToken != nil {
		token, data, err := ParseIDToken(ctx, g.oidc, nil, idToken.(string), tok.AccessToken)
		if err != nil {
			return nil, err
		}
